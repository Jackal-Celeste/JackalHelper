...
namespace Celeste.Mod.TwigHelper.Entities
{
	...
	public class ShroomRefill : Entity
	{
		...
		public ShroomRefill(Vector2 position, bool oneUse, bool refillDash, bool refillStamina, bool legacy)
			: base(position)
		{
			this.legacy = legacy;
			this.oneUse = oneUse;
		...
		public ShroomRefill(EntityData data, Vector2 offset)
			: this(data.Position + offset, data.Bool("oneUse", false), data.Bool("refillDash", true), data.Bool("refillStamina", true), data.Bool("legacy", defaultValue: false))
		{
		}
		...
		{
			if (!TwigModule.Session.HasShroomDash || legacy)
			{
