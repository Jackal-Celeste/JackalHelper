...
{
	[Tracked]
	[CustomEntity("JackalHelper/LavaKevin")]
	public class PyroKevin : Solid
	{
		public enum Axes
		{
			Both,
		...
		public PyroKevin(Vector2 position, float width, float height, Axes axes, bool chillOut = false) : base(position, width, height, false)
		{

			Add(Lava = new LavaRect(width - 2, height - 2, 2));
			Lava.Position += Vector2.One;
			...
			Lava.BigWaveAmplitude = 0f;
			Lava.CurveAmplitude = 10f;
			Lava.SurfaceColor = Color.Transparent;
		...
		public PyroKevin(EntityData data, Vector2 offset)
		...
		{
			face.Color = safe ? Color.Yellow : Color.DarkRed;
			base.Update();
			if (JackalModule.GetPlayer() != null)
			{
				CheckForCryoDash(JackalModule.GetPlayer());
			}
		...
		private bool MoveHCheck(float amount)
		{
			...
			{
				if (amount < 0f && base.Left <= level.Bounds.Left)
				{
					return true;
				}
		...
		private bool MoveVCheck(float amount)
		{
			...
			{
				if (amount < 0f && base.Top <= level.Bounds.Top)
				{
					return true;
				}
				for (int i = 1; i <= 4; i++)
