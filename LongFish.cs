...
using Celeste;
using Celeste.Mod.JackalHelper.Components;
...
{
	[CustomEntity("NotJackalHelper/LongFish")]
	[Tracked]
	public class LongFish : Actor
	{
		private enum States
		{
			Idle,
		...
		public LongFish(Vector2 position, int width, bool faceRight)
			: base(position)
		{
			columns = width / 8;
			base.Collider = new Hitbox(width, 10f);
		...
		public LongFish(EntityData data, Vector2 offset)
			: this(data.Position + offset, data.Int("width"),  data.Bool("right"))
		{
		}
		...
		private void GotoIdle()
		{
			if (state == States.Gone)
			{
				...
				sprite.Play("recover");
				Audio.Play("event:/new_content/game/10_farewell/LongFish_reform", Position);
			}
		...
		private void GotoHit(Vector2 from)
		{
			...
			Audio.Play("event:/new_content/game/10_farewell/LongFish_boop", Position);
		}
        ...
        private void Explode()
		{
			...
			Audio.Play("event:/new_content/game/10_farewell/LongFish_splode", Position);
			sprite.Play("explode");
		...
		public override void Update()
		{
			...
			switch (state)
			{
				case States.Idle:
					{
						...
						{
							Audio.Play("event:/new_content/game/10_farewell/LongFish_shrink", Position);
							sprite.Play("unalert");
						}
						foreach (LongFishCollider component in base.Scene.Tracker.GetComponents<LongFishCollider>())
						{
				...
				case States.Hit:
					...
					foreach (LongFishCollider component2 in base.Scene.Tracker.GetComponents<LongFishCollider>())
					{
				...
				case States.Gone:
					{
						...
						if (goneTimer <= 0.5f)
						{
							...
							{
								Audio.Play("event:/new_content/game/10_farewell/LongFish_return", Position);
							}
		...
		private void Alert(bool restart, bool playSfx)
		{
			if (sprite.CurrentAnimationID == "idle")
			{
				if (playSfx)
				{
					Audio.Play("event:/new_content/game/10_farewell/LongFish_expand", Position);
				}
				sprite.Play("alert");
...
namespace Celeste.Mod.JackalHelper.Components
{
	public class LongFishCollider : Component
	{
		public Action<LongFish> OnCollide;

		public Collider Collider;

		public LongFishCollider(Action<LongFish> onCollide, Collider collider = null)
		...
		public void Check(LongFish LongFish)
		{
			if (OnCollide != null)
			{
				...
				if (LongFish.CollideCheck(base.Entity))
				{
					OnCollide(LongFish);
				}
				base.Entity.Collider = collider;
